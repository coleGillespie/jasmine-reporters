...
(function(global) {
    ...
    exportObject.JUnitXmlReporter = function(options) {
        ...
        function wrapOutputAndWriteFile(filename, text) {
            ...
            prefix += '\n<testsuite name="'+self.namespace.replace(/_/g, ' ');+'" package="" timestamp="'+n+'" time="'+Math.round10(totalTime, -1)+'" id="0" tests="'+totalTests+'" disabled="'+totalTestsDisabled+'" skipped="'+totalTestsSkipped+'" errors="" failures="'+totalTestsFailures+'">';
            prefix += '  <properties><property name="suite.namespace" value="'+self.namespace+'"/></properties>';
            var suffix = '\n</testsuite>';
